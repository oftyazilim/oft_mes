    ...
    try {
      ...
      foreach ($emirler as $emir) {
        ...
        $bomId = DB::connection('pgsql')
          ...
          ->where('pwm.worder_m_id', $emir->isemri_id)
          ->value('pwm.bom_m_id');
        ...
        foreach ($liste as $list) {
          ...
          $qtyNet = (float)($emir->kalan ?? 0);
          $miktar = (float)($emir->miktar ?? 0);
          $kalan  = (float)($emir->kalan ?? 0);

          $hesapMiktari = $miktar > 0 ? $list->qty_prm_exploded * $kalan : 0.0;

          // Log::info("Hesap: $list->qty_prm_exploded * $kalan = $hesapMiktari");
