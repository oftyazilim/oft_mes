...
class UretimMekanikController extends Controller
{
  ...
  // Mekanik duruşları listesi: machine_events tablosundan
  public function MekanikDuruslar(Request $request)
  {
    try {
      $conn = DB::connection('pgsql_oft');
      $q = $conn->table('machine_events as me')
        ->leftJoin('oftt_work_stations as ws', 'ws.wstation_id', '=', 'me.wstation_id')
        ->select([
          'me.id',
          DB::raw("concat_ws('-', ws.wstation_code::text, NULLIF(ws.description::text, '')) as istasyon"),
          'event_ts as durum_bas_tarihi',
          'end_ts as durum_bit_tarihi',
          // Bitiş null ise anlık süre: NOW - event_ts, dakika cinsinden
          DB::raw("ROUND((CASE WHEN end_ts IS NULL THEN (EXTRACT(EPOCH FROM (NOW() - event_ts)))::numeric ELSE COALESCE(duration::numeric, 0) END) / 60, 2) AS durum_suresi"),
          'state as durum',
          'break_reason_code as durus_sebebi_kodu',
          'break_description as durus_sebebi',
          'is_emri_no',
          'urun_kodu',
          'urun_adi',
          'operator_id as personel_id',
          'work_order_id as is_emri_id',
          'product_id as urun_id',
        ])
        // İstenilen filtre: state alanı üzerinden
        ->whereIn('state', ['DOWN']);
      ...
      $to = $request->query('to');
      if ($from) {
        $q->whereDate('event_ts', '>=', $from);
      }
      if ($to) {
        $q->whereDate('event_ts', '<=', $to);
      }
      ...
      $rows = $q
        ->whereRaw(
          "(CASE WHEN end_ts IS NULL THEN EXTRACT(EPOCH FROM (NOW() - event_ts)) ELSE COALESCE(duration, 0) END) > ?",
          [$thresholdSeconds]
        )
        ->orderBy('me.id', 'desc')
        ->limit(5000)
      ...
      // Log::info('MekanikDuruslar kayıt sayısı: ' . $rows->count());

      return response()->json([
        'data' => $rows,
      ]);
    } catch (\Throwable $e) {
      Log::error('MekanikDuruslar hata', [
  ...
  {
    $validated = $request->validate([
      'break_reason_code' => 'required|string|max:100',
      'break_description' => 'required|string|max:255',
    ]);
    ...
    try {
      $affected = DB::connection('pgsql_oft')
        ...
        ->where('id', $id)
        ->update([
          'break_reason_code' => $validated['break_reason_code'],
          'break_description' => $validated['break_description'],
          // 'updated_at' => now(),
        ]);
