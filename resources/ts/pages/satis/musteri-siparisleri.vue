<template>
  ...
  <DxPopup v-model:visible="popupTeslimTarihVisible" :width="500" :height="600" :hide-on-outside-click="true"
    :show-close-button="true" title="Teslim Tarihini Giriniz">
    <template #content>
      <VCol cols="12">
        <div class="popup-center">
...
const Yenile = async (kapsam: string) => {
  gridData.value = [];
  await getSiparisler(kapsam)
}

const getSiparisler = async (kapsam: string) => {
  ...
  try {
    ...
    gridData.value = response.data.siparisler
  } catch (error) {
...
onMounted(async () => {
  pageTitleStore.setTitle('Müşteri Siparişleri')
  getFirmalar();
  loadGridState()
...
const loadGridState = () => {
  const savedState = localStorage.getItem(`${userData.value.id}_musterispr`)
  if (savedState)
...
const saveGridState = () => {
  ...
  localStorage.setItem(`${userData.value.id}_musterispr`, JSON.stringify(state))

  // console.log("Grid durumu kaydedildi:", state);
}

const onStateResetClick = () => {
  localStorage.removeItem(`${userData.value.id}_musterispr`)
...
const onExporting = (e: DxDataGridTypes.ExportingEvent) => {
  ...
  const worksheet = workbook.addWorksheet('MüşteriSiparisleri')
...
const TeslimTarihKaydet = async () => {
  ...
  const userID = userData.value.id
  try {
    const response = await axios.post('/api/teslimtarihidegistirmusteri', { updateData, userID })
...
const renklendir = (e: any) => {
  if (e.rowType === 'data') {
    const rezervli = Number(e.data.REZERVLI_MIKTAR ?? 0)
    const kalan = Number(e.data.KALAN_MIKTAR ?? 0)
    const sevkTarihiStr = e.data.teslim_tarihi

    // Arkaplanı sıfırla (birden fazla koşul varsa üst üste binmesin)
    e.rowElement.style.backgroundColor = ''

    // 1. Eşitlik durumu -> açık yeşil
    if (rezervli === kalan) {
      e.rowElement.style.backgroundColor = '#a7f2bc' // açık yeşil
    }

    // 2. Sevk tarihi geçmişse -> açık kırmızı
    if (sevkTarihiStr) {
      const sevkTarihi = new Date(sevkTarihiStr)
      const simdi = new Date()
      sevkTarihi.setHours(0, 0, 0, 0)
      simdi.setHours(0, 0, 0, 0)

      if (sevkTarihi < simdi) {
        e.rowElement.style.backgroundColor = '#f7c8c8' // açık kırmızı
      }
    }
  }
}
