...
const items = ref<any[]>([]);
const originalItems = ref<Record<string, any>>({});
...
//     if (aktifSaat) {
//       const Mola = MOLALAR.find(m => m.saat === aktifSaat)
//       if (Mola) {
...
//           item.status = original?.status !== 'MOLA' ? original?.status : 'Çalışıyor'
//           item.baslikArkarenk = original?.baslikArkarenk
//           aktifMolalar.value[item.isemriNo] = ''
...
//             userId: userData.value.id,
//             personelSayisi: 0,
//             selectedDurus: { break_reason_code: '', description: '' },
//             guid: item.guid || null,
//           })
//         }
//       } else {
