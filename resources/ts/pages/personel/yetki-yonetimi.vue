                    ...
                    <VCardText>
                        <div class="d-flex flex-wrap gap-4">
                            <div style="max-inline-size: 380px; min-inline-size: 320px;">
                                ...
                                <VList lines="two" class="border rounded" style="overflow: auto; max-block-size: 60vh;">
                                ...
                                <VCard variant="tonal" class="mb-4">
                                    ...
                                    <VCardText>
                                        <div class="d-flex align-center gap-3 flex-wrap">
                                            <VSelect v-model="userRoles" :items="allRoles" item-title="name"
                                                item-value="name" label="Roller" multiple chips clearable hide-selected
                                        ...
                                        <div class="d-flex gap-6 flex-wrap">
                                            <div style="min-inline-size: 260px;">
                                                <div class="d-flex align-center justify-space-between">
                                                    ...
                                                    <VTextField v-model="userPermSearch" label="Ara" density="compact"
                                                        hide-details="auto" clearable style="inline-size: 160px;" />
                                                </div>
                                                <VList class="border rounded"
                                                    style="overflow: auto; max-block-size: 36vh;">
                                                    <VListItem v-for="p in filteredAssignedPerms" :key="p.id">
                                                        <div class="d-flex align-center justify-space-between w-100">
                                                            <span>{{ p.name }}</span>
                                                            <VBtn size="small" variant="text" color="error"
                                                                @click="removePerm(p.id)">Kaldır</VBtn>
                                                        </div>
                                                    </VListItem>
                                                    <VListItem v-if="filteredAssignedPerms.length === 0"
                                                        title="İzin yok" />
                                                </VList>
                                            </div>
                                            <div style="min-inline-size: 260px;">
                                                    ...
                                                    <VListItem v-for="p in filteredAvailablePerms" :key="p.id">
                                                        <div class="d-flex align-center justify-space-between w-100">
                                                            ...
                                                            <VBtn size="small" variant="text" color="primary"
                                                                @click="addPerm(p.id)">Ekle</VBtn>
                                                        </div>
                                                    </VListItem>
                                                    <VListItem v-if="filteredAvailablePerms.length === 0"
                                                        title="İzin yok" />
                                                </VList>
                                            </div>
                                        </div>
                    ...
                    <VCardText>
                        <div class="d-flex gap-4 flex-wrap">
                            <div style="max-inline-size: 380px; min-inline-size: 320px;">
                                ...
                                <VTextField v-model="roleSearch" label="Rol ara" density="compact" clearable
                                    class="mb-2" />
                                <VList class="border rounded" style="overflow: auto; max-block-size: 60vh;">
                                ...
                                <VCard variant="tonal">
                                    ...
                                    <VCardText>
                                        <div class="d-flex align-center gap-3 flex-wrap mb-3">
                                            <VSelect v-model="selectedTemplateKey" :items="permTemplates"
                                                item-title="label" item-value="key" label="Şablon uygula"
                                                density="compact" style="min-inline-size: 220px;" />
                                            <VBtn :disabled="!selectedTemplateKey" color="secondary" variant="tonal"
                                                @click="applyTemplate">Uygula</VBtn>
                                        </div>
                                        <div class="d-flex align-center gap-3 flex-wrap">
                                            <VCombobox v-model="rolePerms" :items="allPerms" item-title="name"
                                                item-value="name" label="İzinler" multiple chips clearable hide-selected
                                                style="min-inline-size: 320px;" />
                                            <VBtn :disabled="!selectedRoleId" color="primary" @click="saveRolePerms">
                                                Kaydet</VBtn>
                                            <VBtn :disabled="!selectedRoleId" color="error" variant="tonal"
                                                @click="deleteRole">Rolü Sil</VBtn>
                        ...
                        <div class="d-flex gap-4 flex-wrap">
                            <div class="flex-grow-1" style="max-inline-size: 560px; min-inline-size: 380px;">
                                <div class="d-flex gap-2 mb-2 flex-wrap">
                                    ...
                                    <VSelect v-model="permSubject" :items="subjects" label="Konu (Sayfa/Modül)"
                                        density="compact" style="min-inline-size: 220px;" />
                                    <VBtn color="primary" @click="createCompositePerm">İzin Oluştur</VBtn>
                                </div>
                                <VTextField v-model="customPerm" label="Özel izin adı (örn: read_montaj)"
                                    density="compact" />
                                <div class="mt-2">
                                ...
                                <VList class="border rounded" style="overflow: auto; max-block-size: 52vh;">
                                    <VListItem v-for="p in filteredPerms" :key="p.id">
                                        <div class="d-flex align-center justify-space-between w-100">
                                            <span>{{ p.name }}</span>
                                            <VBtn size="small" color="error" variant="text"
                                                @click="removeCustomPerm(p.id)">Sil</VBtn>
                                        </div>
                                    </VListItem>
                                </VList>
                            </div>
...
const subjects = ['genel', 'montaj', 'planlama', 'kalite', 'depo', 'satinalma', 'satis', 'users', 'roles']
