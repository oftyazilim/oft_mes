      ...
      <VCol cols="12" class="mt-2 pa-1 pe-2 pt-0">
        ...
        <div id="liste" style="margin-block: -10px 10px;">
          ...
          <DxDataGrid id="gridSA" ref="dataGridRefSA" :key="gridKeySA" class="grid" :data-source="gridDataSA"
            ...
            :auto-navigate-to-focused-row="true" width="100%" @exporting="onExportingSA">
            ...
            <DxColumn data-field="diger_depo" caption="DİĞER DEPO" data-type="number" :visible="true" :width="120"
              :format="{ type: 'fixedPoint', precision: 0, thousandsSeparator: ',', }" />
            <DxColumn data-field="bakiye" caption="BAKİYE" data-type="number" :visible="true" :width="120" :format="{
              type: 'fixedPoint',
              precision: 0,
              thousandsSeparator: ',',
            }" sort-order="asc" />
            <DxColumn data-field="satin_alma" caption="SATINALMA" data-type="number" :visible="true" :width="100"
              :format="{
                type: 'fixedPoint',
                precision: 0,
                thousandsSeparator: ',',
              }" />
            <DxColumn data-field="talep" caption="TALEPLER" data-type="number" :visible="true" :width="100" :format="{
              type: 'fixedPoint',
              precision: 0,
              thousandsSeparator: ',',
            }" />
...
const dataGridRefSA = ref<DxDataGrid | null>(null)
...
document.title = "OFT - İhtiyaç Listesi";
...
const getSatinAlma = async () => {
  ...
  try {
    ...
    gridDataSA.value = response.data.emirler
...
onMounted(async () => {
  pageTitleStore.setTitle('Malzeme İhtiyaç Listesi (Satınalma)')
  loadGridState()
  getMerkezler()
...
const loadGridStateS = () => {
  const savedState = localStorage.getItem(`${userData.value.id}_sip_malzemeler`)
  if (savedState)
    dataGridRefSA.value?.instance.state(JSON.parse(savedState))

  // console.log("Grid durumu yüklendi:", savedState);
}

const saveGridStateS = () => {
  const state = dataGridRefSA.value?.instance.state()

  localStorage.setItem(`${userData.value.id}_sip_malzemeler`, JSON.stringify(state))

  // console.log("Grid durumu kaydedildi:", state);
}

const onStateResetClickS = () => {
  localStorage.removeItem(`${userData.value.id}_sip_malzemeler`)
  dataGridRefSA.value!.instance!.state(null)
}
...
const onContextMenuPreparingS = (e: any) => {
  if (e.rowIndex === -1)
    return
  const data = e.row!.data!
  selectedRow.value = data
...
const onExportingSA = (e: DxDataGridTypes.ExportingEvent) => {
  const workbook = new Workbook()
  const worksheet = workbook.addWorksheet('SiparisIhtiyacListesi')
...
function itemClickS({ itemData }: DxContextMenuTypes.ItemClickEvent) {
  if (!itemData?.items) {
    switch (itemData?.text) {
      case 'Yenile':
        YenileMalzemeler()
        break;
      case 'İş Emirleri Detayları':
        stokId.value = selectedRow.value.item_id
        eksikStokKodu.value = selectedRow.value.stok_kodu
        eksikStokAdi.value = selectedRow.value.stok_adi
        popupIsEmirleriGosterVisible.value = true
...
type DagilimRow = { item_id: number;[key: string]: any }
const filtrelenmisEmirler = (): DagilimRow[] => {
  return gridDataDagilim.value.filter((item: DagilimRow) => item.item_id === stokId.value)
}
