...
export const canNavigate = (to: RouteLocationNormalized) => {
  ...
  if (targetRoute?.meta?.action && targetRoute?.meta?.subject) {
    ...
    console.log(`Checking ${targetRoute.meta.action} on ${targetRoute.meta.subject}:`, canAccess)
    return canAccess
  }
  ...
  const hasAnyAccess = to.matched.some(route => ability.can(route.meta.action, route.meta.subject))
