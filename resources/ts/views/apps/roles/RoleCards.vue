...
// 👉 Roles List - API'den alınacak
const roles = ref<Roles[]>([])
const loading = ref(false)

// API'den roles verilerini al
const fetchRoles = async () => {
  try {
    ...
    roles.value = rolesData.map((role: any) => ({
      role: role.name,
      users: [avatar1, avatar2, avatar3, avatar4], // Şimdilik sabit avatar'lar
      details: {
        name: role.name,
